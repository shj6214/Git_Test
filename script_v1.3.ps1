...
# csv 파일 임포트 선언
$resourceinfos = Import-csv -Path "C:\Users\shj\shtest\Git_Test\resource.csv"
...
for ($i=0; $i -lt $VMs.length; $i++){
    ...
    # NIC 생성 (NIC 01 ~ 03 3ea)
    if ($Nics.name[$i] -eq "NIC01") { # VM-KC-01 용 NIC에 PIP 연결 (nic <-> pip)
        $new_nic = New-AzNetworkInterface -Name $nics.name[$i] -ResourceGroupName $RG.name -Location $LocationName -SubnetId $new_vnet.Subnets[0].id -PublicIpAddressId $NEW_PIP.id -PrivateIpAddress $nics.ipaddress[$i]
    }
    else{ # VM-KC-02, VM-KC-03 NIC 연결
        $new_nic = New-AzNetworkInterface -Name $nics.name[$i] -ResourceGroupName $RG.name -Location $LocationName -SubnetId $new_vnet.Subnets[0].id -PrivateIpAddress $nics.ipaddress[$i]
    }
